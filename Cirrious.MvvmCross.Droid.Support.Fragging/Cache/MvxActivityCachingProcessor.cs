...
namespace Cirrious.MvvmCross.Droid.Support.Fragging.Cache
{
    public class MvxActivityCachingProcessor
    {
        private readonly Func<FragmentManager> _supportFragmentManagerProviderProvider;
        private readonly Func<Context> _activityContextProvider;

        protected FragmentManager SupportFragmentManager => _supportFragmentManagerProviderProvider();
        ...
        public MvxActivityCachingProcessor(Func<FragmentManager> supportFragmentManagerProvider, Func<Context> activityContextProvider)
        {
            _supportFragmentManagerProviderProvider = supportFragmentManagerProvider;
        ...
        public void RestoreFragmentsOnPostCreate(Bundle savedInstanceState)
        {
        ...
        private void RestoreViewModelsFromBundle(IMvxJsonConverter serializer, Bundle savedInstanceState)
        {
        ...
        public void CacheFragmentsOnActivityStateSave(Bundle outState)
        {
            if (_lookup.Any())
            {
        ...
        public void ShowFragment(string tag, int contentId, Bundle bundle = null, bool addToBackStack = false, bool forceReplaceFragment = false)
        {
            ...
            {
                fragInfo.CachedFragment = Fragment.Instantiate(_activityContextProvider(), FragmentJavaName(fragInfo.FragmentType),
                    bundle);

                ft.Add(fragInfo.ContentId, fragInfo.CachedFragment, fragInfo.Tag);
            }
            else
                ft.Attach(fragInfo.CachedFragment); // never executed DEAD CODE!! <- @thefex
        ...
        public void HandleFragmentsStateOnBackPressed()
        {
        ...
        public void CloseFragment(string tag, int contentId)
        {
            var frag = SupportFragmentManager.FindFragmentById(contentId);
            if (frag == null) return;
        ...
        public event Action<string, FragmentTransaction> BeforeFragmentChanging;
        public event Action<string, FragmentTransaction> FragmentChanging;


        protected virtual void OnBeforeFragmentChanging(string arg1, FragmentTransaction arg2)
        {
            BeforeFragmentChanging?.Invoke(arg1, arg2);
        }

        protected virtual void OnFragmentChanging(string arg1, FragmentTransaction arg2)
        {
            FragmentChanging?.Invoke(arg1, arg2);
        }
    }
}